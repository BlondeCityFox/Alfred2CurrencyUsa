...
// get "from"
$title = 'Calculating from '.$fromText;
...
// get "to"
foreach ($toCurrencies as $index=>$to) {
	if ($from == $to)
		continue;
	$rate = currencyrate($from, $to);
	$value = $rate * $q;
	add('rate', $toCurrenciesSymbols[$index].number_format($value, 2, '.', ','), 'Factor: 1 '.$from.' = '.number_format($rate, 6, '.', ',').' '.$to, 'icons/'.strtolower($to).'.png');
}
